...
import skillbox.javapro11.api.response.EmailResponse;
...
public class AccountController {
    ...
    @PutMapping("/email")
    public ResponseEntity<CommonResponse> emailChange(@RequestBody EmailResponse emailResponse){
        LOGGER.trace("/api/v1/account/email");

        String message = accountService.changePersonEmail(emailResponse.getEmail());
        return new ResponseEntity<>(new CommonResponse(message), HttpStatus.OK);
    }
